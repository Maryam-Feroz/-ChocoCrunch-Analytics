...
# --- Tabs ---
tabs = st.tabs(["Introduction", "Product Info", "Nutrient Info", "Derived Metrics","Join Queries"])
...
with tabs[0]:    
    ...
    <!-- Domain Subtitle: Bold, Elegant, Slightly Larger -->
    <h3 style='
        text-align: center; 
        color: #D2691E; 
        ...
        font-size: 34px;
    '>
        Domain : Nutrition & Chocolate Market Analysis
    </h3>
""", unsafe_allow_html=True)
    ...
    st.markdown(
        "Welcome to **ChocoCrunch Analytics**! Here, data meets deliciousness 🍫 and insights uncover the story behind your favorite chocolate products. "
        "Dive in to explore calorie-packed treats, sugar trends, brand comparisons, and how processed your chocolates really are! 💡"
    )
    
    st.markdown("### 🔍 What stakeholders can discover:")
    st.markdown(
        """
        - Identify **high-calorie & high-sugar products**  
        - Track **ultra-processed chocolate trends** via NOVA classification 
        - Compare **brands based on healthiness metrics** 
        - Discover **calorie & sugar classes** across products 
        - Get an **interactive dashboard experience** to make informed decisions 
        """
    )
    
    st.markdown("### 📝 What users can explore:")
    st.markdown(
        """
        - **Product Info:** Browse chocolate products & their details 
        - **Nutrient Info:** Check nutrition facts & values  
        - **Derived Metrics:** See calculated metrics like average carbs, calories per brand 
        - **Join Queries:** Explore insights from combined datasets and trends 
        """
    )
    
    st.markdown("<hr style='border:2px solid #8B0000;'>", unsafe_allow_html=True)
    
    st.success("Get ready to uncover the sweet secrets of the chocolate world!🍬✨")
